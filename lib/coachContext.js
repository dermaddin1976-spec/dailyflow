...
export async function buildCoachContext(userId, user, readiness) {
  ...
  const start = dateStr(-6), end = today;

  // The model was calling today's meals "yesterday's" because the raw dates
  // below gave it nothing to anchor "today" to — it had to guess which row
  // was current. Labeling the two dates that matter removes that guess.
  ...
  lines.push(
    `Today's date is ${today}. Every date below is YYYY-MM-DD and, where it matters, marked (today) or ` +
    '(yesterday) \u2014 always go by those explicit dates and labels, never guess or assume which row is "today".',
  );
  ...
  lines.push(sleepRows.length ? sleepRows.map(r => `${labelDate(r.date)}: ${r.hours}h`).join('; ') : 'No sleep logged.');

  lines.push('\nLast 7 days of training:');
  lines.push(
    workoutRows.length
      ? workoutRows.map(r => `${labelDate(r.date)}: ${r.type || 'workout'}, ${r.minutes}min${r.intensity ? `, intensity ${r.intensity}/10` : ''}`).join('; ')
      : 'No training logged.',
  );
  ...
  if (mealRows.length) {
    ...
    lines.push(Object.entries(byDate).map(([d, t]) => `${labelDate(d)}: ${t.calories} cal, ${t.protein}g protein (${t.count} meals)`).join('; '));
  } else {
  ...
  lines.push(studyDays.length ? studyDays.map(r => `${labelDate(r.date)}: ${r.minutes}min`).join('; ') : 'No study logged.');
