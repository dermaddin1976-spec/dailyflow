...
export async function getUserFromToken(token) {
  ...
  const row = await db.prepare(`
    SELECT s.expires_at as expires_at, u.id as id, u.email as email, u.name as name,
           ...
           u.google_calendar_email as google_calendar_email,
           u.apple_health_token as apple_health_token, u.banned_at as banned_at,
           u.grocery_store as grocery_store, u.kitchen_tools as kitchen_tools
    FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ?
  `).get(token);
  if (!row) return null;
  if (new Date(row.expires_at) < new Date()) return null;
  // A banned account's sessions are deleted when the ban is applied, but a
  // request already in flight (or a token issued a split second earlier)
  // could still slip through — this is the second, always-checked gate.
  if (row.banned_at) {
  ...
  return {
    ...
    google_calendar_connected: !!row.google_calendar_email,
    ...
    kitchen_tools: row.kitchen_tools,
  };
}
