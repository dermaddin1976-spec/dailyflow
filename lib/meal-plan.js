...
function buildSingleServingPrompt(answers, targets) {
  const {
    scope, mealType, diet, dietOther, allergies, cuisines,
    cookingTime, servings, currency, extraNotes,
  } = answers;

  const kind = scope === 'snack' ? 'snack' : 'meal';
  ...
  return [
    `You are a nutrition planning assistant. The user wants ONE ${kind} right now, not a multi-day plan.`,
    ``,
    `For rough context only, their daily targets are about ${targets.calories} calories, ${targets.protein}g protein, ${targets.carbs}g carbs, ${targets.fat}g fat — use this only to judge a sensible portion for a single ${kind}, not as something this one ${kind} needs to hit on its own.`,
    ...
    cuisines && cuisines.trim() ? `Preferred cuisines or foods to lean into where it fits: ${cuisines.trim()}.` : '',
    `Cooking time budget: ${COOKING_TIME_LABELS[cookingTime] || COOKING_TIME_LABELS.quick}.`,
    ...storeAndToolsLines(answers),
    `Cook for ${servings || 1} serving${(servings || 1) === 1 ? '' : 's'}.`,
    extraNotes && extraNotes.trim()
      ...
      : '',
    ``,
    `Give exactly ONE ${kind} with a short recognizable name, a one-sentence description, and its calories/protein/carbs/fat for one serving.`,
    `Then list the ingredients it actually needs as a shopping list (skip pantry basics the user would obviously already`,
...
function buildFullPlanPrompt(answers, targets) {
  const {
...
function buildPrompt(answers, targets) {
  if (answers.scope === 'meal' || answers.scope === 'snack') {
    return buildSingleServingPrompt(answers, targets);
  }
  return buildFullPlanPrompt(answers, targets);
}
