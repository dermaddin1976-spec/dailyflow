...
async function runMigrations() {
  ...
  // All ~35 of these were previously individual awaited round trips to
  // Neon — since ensureInit() runs before every single query, that meant
  // every cold serverless instance paid for ~35 sequential network hops
  // before its very first real query could even start. They're all
  // idempotent (IF NOT EXISTS / UPDATE ... WHERE ... IS NULL), and a single
  // multi-statement string sent through one pool.query() call runs them in
  // this same order in one round trip (same trick already used for the
  // CREATE TABLE block above) — so this now costs one hop instead of ~35.
  // Wrapped in try/catch, same as the old per-statement safeAlter, so an
  // unexpected failure here logs instead of breaking every request.
  try {
    await pool.query(`
      ALTER TABLE flashcards ADD COLUMN IF NOT EXISTS box INTEGER;
      ALTER TABLE flashcards ADD COLUMN IF NOT EXISTS notebook TEXT;
      UPDATE flashcards SET notebook = source_title WHERE notebook IS NULL;
      ALTER TABLE flashcards ADD COLUMN IF NOT EXISTS due_date TEXT;
      ALTER TABLE flashcards ADD COLUMN IF NOT EXISTS last_reviewed TEXT;
      UPDATE flashcards SET box = 1 WHERE box IS NULL;
      UPDATE flashcards SET due_date = CURRENT_DATE::text WHERE due_date IS NULL;

      ALTER TABLE meal_logs ADD COLUMN IF NOT EXISTS carbs INTEGER;
      ALTER TABLE meal_logs ADD COLUMN IF NOT EXISTS fat INTEGER;
      ...
      ALTER TABLE workout_logs ADD COLUMN IF NOT EXISTS strava_activity_id TEXT;
      ALTER TABLE workout_logs ADD COLUMN IF NOT EXISTS distance_km REAL;
      ALTER TABLE workout_logs ADD COLUMN IF NOT EXISTS title TEXT;
      ...
      ALTER TABLE users ADD COLUMN IF NOT EXISTS google_calendar_token_expires_at BIGINT;
      CREATE UNIQUE INDEX IF NOT EXISTS calendar_events_google_uniq ON calendar_events (user_id, google_event_id) WHERE google_event_id IS NOT NULL;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS apple_health_token TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS banned_at TIMESTAMPTZ;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS grocery_store TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS kitchen_tools TEXT;
      ALTER TABLE meal_plans ADD COLUMN IF NOT EXISTS regen_history_json TEXT;
      ALTER TABLE meal_logs ADD COLUMN IF NOT EXISTS photo_data_url TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS last_reminder_date TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS readiness_tip TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS readiness_tip_date TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS insight_text TEXT;
      ALTER TABLE users ADD COLUMN IF NOT EXISTS insight_date TEXT;
    `);
  } catch (e) {
    console.warn('[db] batched migration step failed (continuing):', e.message);
  }
}
