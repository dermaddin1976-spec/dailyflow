...
async function runMigrations() {
  ...
  await safeAlter('ALTER TABLE flashcards ADD COLUMN IF NOT EXISTS notebook TEXT');
  // Every existing deck becomes its own single-source notebook by default —
  // this only touches rows a prior run hasn't already backfilled.
  await safeAlter('UPDATE flashcards SET notebook = source_title WHERE notebook IS NULL');
  ...
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS google_calendar_email TEXT');
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS google_calendar_access_token TEXT');
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS google_calendar_refresh_token TEXT');
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS google_calendar_token_expires_at BIGINT');
  await safeAlter('CREATE UNIQUE INDEX IF NOT EXISTS calendar_events_google_uniq ON calendar_events (user_id, google_event_id) WHERE google_event_id IS NOT NULL');
  ...
  await safeAlter('ALTER TABLE meal_logs ADD COLUMN IF NOT EXISTS photo_data_url TEXT');
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS last_reminder_date TEXT');
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS readiness_tip TEXT');
  await safeAlter('ALTER TABLE users ADD COLUMN IF NOT EXISTS readiness_tip_date TEXT');
}
