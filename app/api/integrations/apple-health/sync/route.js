...
export async function POST(request) {
  ...
  const user = await db.prepare('SELECT id FROM users WHERE apple_health_token=?').get(token);
  if (!user) {
  ...
  if (body.sleep_hours != null && Number(body.sleep_hours) >= 0) {
    ...
    const existing = await db.prepare('SELECT id FROM sleep_logs WHERE user_id=? AND date=?').get(user.id, date);
    if (existing) {
      await db.prepare('UPDATE sleep_logs SET hours=?, quality=?, note=? WHERE id=?')
        .run(hours, quality, 'Synced from Apple Health', existing.id);
    } else {
      await db.prepare('INSERT INTO sleep_logs (user_id, date, hours, quality, note) VALUES (?, ?, ?, ?, ?)')
  ...
  if (body.weight_kg != null && Number(body.weight_kg) > 0) {
    ...
    await db.prepare('DELETE FROM weight_logs WHERE user_id=? AND date=?').run(user.id, date);
    await db.prepare('INSERT INTO weight_logs (user_id, date, weight_kg, source) VALUES (?, ?, ?, ?)')
      .run(user.id, date, weightKg, 'apple_health');
    await db.prepare('UPDATE users SET weight_kg=? WHERE id=?').run(weightKg, user.id);
    synced.push('weight');
  }
  ...
  if (body.workout_minutes != null && Number(body.workout_minutes) > 0) {
    ...
    await db.prepare("DELETE FROM workout_logs WHERE user_id=? AND date=? AND note='Synced from Apple Health'")
      .run(user.id, date);
