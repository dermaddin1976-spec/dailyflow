...
export async function POST(request) {
  ...
  db.prepare('INSERT INTO study_logs (user_id, date, subject, minutes, focus, note) VALUES (?, ?, ?, ?, ?, ?)')
...
export async function GET() {
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: 'Not signed in.' }, { status: 401 });
  const rows = db.prepare('SELECT * FROM study_logs WHERE user_id=? ORDER BY id DESC LIMIT 20').all(user.id);
